...
Nivel1::Nivel1(Dificultad dificultadSeleccionada)
{
    ...
    configs[NORMAL] = {2.0f, 0.7f, 0.02f, 400.0f};
    configs[DIFICIL] = {0.55f, 0.18f, 0.08f, 650.0f};
...
void Nivel1::actualizar()
{
    ...
    // obstaculos
    if (tiempoGeneracion >= intervaloActual && tiempoJuego < 48.0f)
    {
        generarObstaculo();
        tiempoGeneracion = 0.0f;
    }
...
void Nivel1::renderizar(QPainter& painter)
{
    ...
    if (finNivel)
    {
        ...
        if (llegoAlFinal)
            painter.drawText(250, 250, "NIVEL COMPLETADO");
        else
            painter.drawText(280, 250, "TIEMPO AGOTADO");
...
void Nivel1::escalarDificultad(float dt)
{
    if (intervaloActual > config.intervaloMinimo)
        intervaloActual -= config.aceleracion * dt * 30.0f;
...
bool Nivel1::colision(Entidad* a, Entidad* b)
{
    ...
    Vector3 posJugador = a->getPosicion();
    QRect rectJugador(
        static_cast<int>(512 + posJugador.x * 180 - 25),
        400,
        50,
        50
        );
...
{
    Vector3 posJugador = jugador->getPosicion();
    QRect rectJugador(
        static_cast<int>(512 + posJugador.x * 180 - 25),
        400,
        50,
    ...
    return rectJugador.intersects(rectFinal);
}
...
void Nivel1::dibujarTemporizador(QPainter& painter)
{
    ...
    QString texto = QString::number((int)std::ceil(tiempoRestante));
...
{
    // meta
    int screenX = static_cast<int>(xRealFinal);
    int screenY = static_cast<int>(yRealFinal + scrollMundo);

    painter.setBrush(QColor(255, 140, 0));
    painter.setPen(Qt::white);
    painter.drawRect(screenX, screenY, 70, 70);
}
